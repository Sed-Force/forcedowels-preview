...
(function () {
  ...
  // Destination labels (if present in your markup)
  ...
  function updateBadge(items) {
    ...
    for (const it of items) {
      if (it.type === 'bulk') total += it.units; // design choice: show units aggregate
  ...
  function render() {
    ...
    // If we already had a chosen shipping rate, keep grand total in sync
  ...
  function onStep(delta, ev) {
    ...
    saveCart(items);
    // Changing cart invalidates shipping rate (price depends on size/weight)
    clearStoredRate();
    clearRatesUI();
    render();
  }
  ...
  // Simple prompt for destination (kept minimal so you can style/replace later)
  ...
  if (btnChangeDest) {
    btnChangeDest.addEventListener('click', () => {
      ...
      if (dest) {
        // destination change invalidates existing shipping selection
        clearStoredRate();
        clearRatesUI();
  ...
  async function requestRates() {
    ...
    if (btnGetRates) {
      ...
      try {
        ...
        btnGetRates.textContent = 'Get Rates';
      }
    }
  }
  ...
  function renderRates(rates) {
    ...
    // Auto-select cheapest (first, since API sorted already)
    onSelectRate(rates[0]);
  }

  if (btnGetRates) {
    btnGetRates.addEventListener('click', requestRates);
  }
  ...
  if (btnCheckout) {
    btnCheckout.addEventListener('click', async () => {
      try {
        ...
        // (If your Stripe session currently handles shipping itself, you can relax this.)
        const rate = getStoredRate();
        if (!rate) {
          if (!confirm('No shipping rate selected. Continue without shipping?')) {
            btnCheckout.disabled = false;
            btnCheckout.textContent = prev;
            return;
          }
        ...
        // Persist selection for the success page, emails, etc.
        ...
        const res = await fetchWithTimeout('/api/checkout', {
          ...
          body: JSON.stringify({
            items,
            // Optionally pass shipping to your backend if you want stripe to add it:
